...
def onnx2tensorrt(
    ...
    trt_engine = onnx2trt(
        onnx_model, opt_shape_dict, fp16_mode=fp16_mode, max_workspace_size=max_workspace_size
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--trt-file", type=str, default="tmp.trt", help="Filename of the output TensorRT engine"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify the outputs of ONNXRuntime and TensorRT"
    )
    parser.add_argument(
        "--shape", type=int, nargs="+", default=[224, 224], help="Input size of the model"
    )
    parser.add_argument(
        "--max-batch-size", type=int, default=1, help="Maximum batch size of TensorRT model."
    )
    parser.add_argument("--fp16", action="store_true", help="Enable fp16 mode")
    parser.add_argument(
        "--workspace-size", type=int, default=1, help="Max workspace size of GPU in GiB"
    )
