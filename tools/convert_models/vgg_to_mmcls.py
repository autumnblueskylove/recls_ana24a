...
def get_layer_maps(layer_num, with_bn):
    ...
    if with_bn:
        ...
        layer_maps["conv"] = {
            layer_idx: new_layer_idx for layer_idx, new_layer_idx in zip(layer_idxs, new_layer_idxs)
        }
            ...
            for layer_key, maps in layer_maps.items():
                if layer_idx in maps:
                    ...
                    new_key = delimiter.join(
                        [prefix, "features", str(new_layer_idx), layer_key, weight_type]
                    )
...
def main():
    ...
    parser.add_argument(
        "--layer_num", type=int, choices=[11, 13, 16, 19], default=11, help="number of VGG layers"
    )
