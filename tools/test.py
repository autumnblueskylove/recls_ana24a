...
try:
    ...
    warnings.warn("wrap_fp16_model from mmcls will be deprecated." "Please install mmcv>=1.1.4.")
...
def parse_args():
    ...
    parser.add_argument(
        "--device", choices=["cpu", "cuda"], default="cuda", help="device used for testing"
    )
...
def main():
    ...
    if "CLASSES" in checkpoint.get("meta", {}):
        ...
        warnings.warn(
            "Class names are not saved in the checkpoint's " "meta data, use imagenet by default."
        )
    ...
    if rank == 0:
        results = {}
        if args.metrics:
            eval_results = dataset.evaluate(outputs, args.metrics, args.metric_options)
