...
class DataPlatformReader(object):
    ...
    def check_object(self):
        search_query = 'SELECT cx,cy,width,height,angle,label_code_id from data_platform.tbl_sia_labels WHERE '
    ...
    def get_obj_info_by_label_id(self, label_id):
        search_query = 'SELECT cx,cy,width,height,angle,label_code_id,scene_id from data_platform.tbl_sia_labels WHERE '
        condition_query = f"id='{label_id}'"
        tot_query = search_query + condition_query
        return self.execute(tot_query)[0]

    def get_scene_info_by_scene_id(self, scene_id):
        search_query = 'SELECT basepath from data_platform.tbl_sia_scenes WHERE '
    ...
    def make_dataset(self):
        ...
        for label_id in tqdm(self.object_list):
            ...
            data = dict(scene_path=scene_path,
                        x=x,
                        y=y,
                        w=w,
                        h=h,
