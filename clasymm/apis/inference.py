import numpy as np
import torch
from clasymm.datasets import SceneDataset
...
def inference_classifier_with_scene(model,
    ...
    if infer_pipeline[0].type != 'ConvertSceneToPatch':
        raise RuntimeError(
            f'First test pipeline should be `ConvertSceneToPatch`, '
            f'and followed by pipelines such as `RandomStRetch and `CropInstance`'
        )

    dataset = SceneDataset(scene_path,
                           pipeline=infer_pipeline,
                           objects=objects,
                           object_file=object_file)

    loader_cfg = dict(workers_per_gpu=cfg.data.get('workers_per_gpu', 1),
